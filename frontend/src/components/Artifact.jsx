...
const PanelContent = ({ artifacts, collapsed, onClose, onCollapse, onExpand }) => {
    const [tab, setTab] = useState("code")
    const [activeFile, setActiveFile] = useState(0)
    const [copied, setCopied] = useState(false)

    const files = artifacts[0]?.files || []
    const safeActiveFile = Math.min(activeFile, Math.max(files.length - 1, 0))
        ...
        <>
            {!collapsed ? <div className='flex flex-col h-full bg-[#0d0f14]'>
                ...
                <div className='h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0'>
                    <button className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0' onClick={onClose ?? onCollapse}>
                ...
                {tab === "code" && <div className='flex h-auto border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0'>
                    {
                        files.map((f, index) => (
                            <button
                                ...
                                className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-colors duration-150 border-r border-white/[0.05] relative cursor-pointer bg-transparent   ${safeActiveFile === index ? "text-indigo-400" : "text-slate-500 hover:text-slate-300"}`}
                            >
                                {f?.name}
                                {safeActiveFile === index && <div className='absolute bottom-0 left-0 right-0 h-[2px] bg-indigo-500 rounded-t-full' />}
                ...
                <div className='hidden lg:flex h-full border-l border-white/[0.06] bg-[#0d0f14] flex-col items-center py-4 gap-3 shrink-0'>
                    <button className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0' onClick={onExpand}>
    ...
    return (
        <>
            ...
            <AnimatePresence>
                {mobileOpen && <>
                    ...
                    <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.25, ease: "easeInOut" }} className="lg:hidden fixed inset-y-0 right-0 z-50 w-[88vw] max-w-[420px] border-l border-white/[0.06] overflow-hidden">
                        <PanelContent artifacts={artifacts} collapsed={collapsed} onClose={() => setMobileOpen(false)} />
                    </motion.div>
            ...
            <motion.div
                ...
                <PanelContent artifacts={artifacts} collapsed={collapsed} onCollapse={() => setCollapsed(true)} onExpand={() => setCollapsed(false)} />
            </motion.div>
        </>
    )
}
